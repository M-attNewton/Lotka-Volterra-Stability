...
% a = [1; 1; 1]; % [xy; xz; yz] terms
% c = 1; % Cubic terms
% b = 1; % Normalise system so birth rate is 1

% % True vector field
% x_dot = x*(b - d*x - a(1)*y - a(2)*z - c*y*z);
% y_dot = y*(b - a(1)*x - d*y - a(3)*z - c*x*z);
% z_dot = z*(b - a(2)*x - a(3)*y - d*z - c*x*y);
% 
% % Remove zero terms to speed up simultanious equation solver
% x_dot = (b - d*x - a(1)*y - a(2)*z - c*y*z);
% y_dot = (b - a(1)*x - d*y - a(3)*z - c*x*z);
% z_dot = (b - a(2)*x - a(3)*y - d*z - c*x*y);
...
% Region D
D = (x-sequil(1))^2 + (y-sequil(2))^2 + (z-sequil(3))^2 - 0.3^2;
